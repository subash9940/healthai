...
CREATE TABLE IF NOT EXISTS sos_alerts_v2 (
...
-- If legacy sos_alerts table exists with old schema (lacking client_alert_id), migrate / replace it
DO $$
BEGIN
    IF EXISTS (
        ...
        WHERE table_schema = 'public' AND table_name = 'sos_alerts' AND column_name = 'client_alert_id'
    ) THEN
        DROP TABLE sos_alerts CASCADE;
    END IF;
END $$;
...
DROP TABLE IF EXISTS sos_alerts_v2;
